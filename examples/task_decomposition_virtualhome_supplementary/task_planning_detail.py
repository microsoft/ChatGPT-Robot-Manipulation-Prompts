...
prompt_load_order = ['prompt_role',
                     'prompt_function_detail',
                     'prompt_environment',
                     'prompt_output_format',
                     'prompt_example_detail']
...
class ChatGPT_api:
    ...
    def generate(self, message, environment, is_user_feedback=False):
        ...
        try:
            ...
            self.json_dict = None
            return None
        # modify the json dict to make it more readable
        # mapping = {
        #     "Task0": "WalkTowards",
        #     "Task1": "Grab",
        #     "Task2": "Open",
        ...
        mapping = {
            "PutSurface": "Put",
            "PutContainerWithDoor": "PutIn"
        }
        def convert_task(task):
            task_prefix = task.split('(', 1)[0]
            if task_prefix in mapping:
                return task.replace(task_prefix, mapping[task_prefix])
            else:
                return task
        converted_tasks = list(map(convert_task, self.json_dict['task_cohesion']['task_sequence']))
        #import pdb;pdb.set_trace()
        self.json_dict['task_cohesion']['task_sequence'] = converted_tasks
...
if __name__ == '__main__':
    ...
    dir_name = "out_task_planning_gpt-3.5-turbo-16k_temp=2.0_detail"
    waittime_sec = 30
    max_trial = 5
